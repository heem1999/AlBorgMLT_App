...
MedicalAttachments=[];
  ...
  loadOrderDetails(){
    this.util.startLoad();
    this.api.getDataWithToken("MLT_request/" + this.api.bookid).subscribe(
      (res: any) => {
        if (res.success) {
  ...
  start_processing(order_id){
    ...
    if (localStorage.getItem("Language") == "En") {
      ...
      message='Are you sure you want to start processing this request?';
      yes='Yes!';
      no='No';
    } else {
      header='تنبيه للتأكيد ';
      subHeader='احذر دعنا نؤكد';
      message='هل أنت متأكد أنك تريد بدء معالجة هذا الطلب؟';
      yes='نعم!';
      no='لا';
    }
    ...
    this.alertController.create({
      ...
      buttons: [
       ...
       
        {
          ...
          this.api.postDataWithToken("start_processing",info).subscribe(
              (res: any) => {
                if (res.success) {
                  this.util.dismissLoader();
                  ...
                  if (localStorage.getItem("Language") == "En") {
                      this.util.presentToast("This request is under your processing!");
                    } else {
                      this.util.presentToast("هذا الطلب تحت معالجتك!");
                    }
                }
              },
              (err) => {
  ...
  smaple_collected(order_id){
    var header,subHeader,message,yes,no,note;
    
    if (localStorage.getItem("Language") == "En") {
      header='Confirm Alert';
      subHeader="Beware let's confirm";
      message='Have samples been collected from the patient?';
      yes='Yes!';
      no='No';
      ...
      header='تنبيه للتأكيد ';
      subHeader='احذر دعنا نؤكد';
      message='هل تم جمع العينات من المريض؟';
      yes='نعم!';
      no='لا';
    }
      ...
      buttons: [
       ...
       
        {
          ...
          handler: () => {
            let info: any = {};
            info.order_id = order_id;
          this.api.postDataWithToken("smaple_collected",info).subscribe(
              (res: any) => {
                if (res.success) {
                  this.util.dismissLoader();
                  if (localStorage.getItem("Language") == "En") {
                      this.util.presentToast("Samples were collected successfully!");
                    } else {
                      this.util.presentToast("تم جمع العينات بنجاح!");
                    }
                }
              },
              (err) => {
